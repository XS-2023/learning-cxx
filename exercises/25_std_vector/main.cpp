...
int main(int argc, char **argv) {
    {
        ...
        // sizeof(vec) 是 vector 对象本身的大小，与它存储的元素数量无关。
        // 它通常包含一个指向堆内存的指针、一个size变量和一个capacity变量。
        // 在64位系统上，这通常是 3 * 8 = 24 字节。
    ...
    }
    {
        ...
        }
        {
            ...
            ASSERT(sizeof(vec) == 24, "Fill in the correct value."); // vector对象本身大小不变
            vec.pop_back();
            ASSERT(vec.size() == 5, "Fill in the correct value.");
            ASSERT(sizeof(vec) == 24, "Fill in the correct value."); // vector对象本身大小不变
        }
        ...
        }
        {
            ...
            // 插入和删除的时间复杂度是线性的 O(N)，因为需要移动后面的元素。
        ...
        {
            vec.shrink_to_fit();
            ASSERT(vec.capacity() == 5, "Fill in the correct value."); // 容量收缩到与大小一致
            vec.clear();
            ASSERT(vec.empty(), "`vec` is empty now.");
            ASSERT(vec.size() == 0, "Fill in the correct value."); // size变为0
            ASSERT(vec.capacity() == 5, "Fill in the correct value."); // capacity通常不变
        }
    }
    {
        // 使用构造函数 (count, value) 创建一个包含 48 个 'z' 的 vector
        ...
        ASSERT(sizeof(vec) == 24, "Fill in the correct value.");
        {
            ...
            ASSERT(vec.capacity() == capacity, "Fill in a correct identifier."); // capacity通常不变
        }
        {
            vec.reserve(256);
            ASSERT(vec.size() == 16, "Fill in the correct value."); // size不变
            ASSERT(vec.capacity() == 256, "Fill in the correct value."); // capacity增加
        }
        {
            vec.push_back('a');
            vec.push_back('b');
            vec.push_back('c');
            vec.push_back('d');
            ASSERT(vec.size() == 20, "Fill in the correct value."); // 16 + 4
            ASSERT(vec.capacity() == 256, "Fill in the correct value."); // capacity不变，因为足够
            ASSERT(vec[15] == 'z', "Fill in the correct value."); // 最后一个'z'
